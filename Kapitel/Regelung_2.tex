...
Die direkte Bestimmung des Lenkwinkels mit Hilfe einer Referenztrajektorie \cite{ackermann} ist eher eine Art der Steuerung als eine Regelung. Diese Methode benötigt eine Trajektorie, wie z.~B. in Abschnitt \ref{sec:traj} beschrieben. Weiterhin wird ein möglichst exaktes Fahrzeugmodell (siehe: \ref{sec:model}). Nun wird ein Punkt auf der Trajektorie gewählt, der sich eignet darauf zu regeln. D.h. die Krümmung wie in Gleichung \ref{eq:curv} wird dort bestimmt. Aus dem Einspurmodell nach Ackermann ergibt sich, dass für eine Kurvenfahrt konstanten Radius der laterale Abstand ($R$ siehe Abb.~\ref{fig:ackermann}) zwischen Hinterachse des Fahrzeugs und dem augenblicklichen Krümmungszentrum (engl. ICC) dem Kehrwert der Krümmung $\kappa$ entspricht. 
