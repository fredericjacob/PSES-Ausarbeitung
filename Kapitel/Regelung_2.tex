...
Der PID-Regler (Abb.~\ref{fig:PID-System}) erhält den aktuellen zwischen longitudinaler Achse und der Referenztrajektorie in einem bestimmten Abstand zum Fahrzeug als auszuregelnden Fehler. Dabei besteht der Regler aus drei verschiedenen Elementen. 
...
Dem P-Glied, welches den Fehler linear verstärkt. Einem I-Glied, welches den Fehler über die Zeit aufsummiert und somit ein Gedächtnis hat. I-Glieder eignen sich besonders, um bleibende Regelabweichungen zu verhindern. Weiterhin ist ein kleiner D-Anteil benutzt worden, um schnell auf auftretende Fehler reagieren zu können. D-Glieder verstärken die Änderung des Fehlers. Im speziellen wurde der PID-Regler allerdings so angepasst, dass für den Fall, dass der Fehler am Sinken ist das I-Glied schneller abgebaut wird. Der I-Anteil steigt weiterhin an, selbst wenn der Fehler bereits ausgeregelt wird. Der exakte Entwurf ist in Abbildung \ref{fig:PID} zu sehen.
...
Die direkte Bestimmung des Lenkwinkels mit Hilfe einer Referenztrajektorie \cite{ackermann} ist eher eine Art der Steuerung als eine Regelung. Diese Methode benötigt eine Trajektorie, wie z.~B. in Abschnitt \ref{sec:traj} beschrieben. Weiterhin wird ein möglichst exaktes Fahrzeugmodell (siehe: \ref{sec:model}) benötigt. Nun wird ein Punkt auf der Trajektorie gewählt, der sich eignet darauf zu regeln. An diesem Punkt wird die Krümmung wie in Gleichung \ref{eq:curv} bestimmt. Aus dem Einspurmodell nach Ackermann ergibt sich, dass für eine Kurvenfahrt konstanten Radius der laterale Abstand ($R$ siehe Abb.~\ref{fig:ackermann}) zwischen Hinterachse des Fahrzeugs und dem augenblicklichen Krümmungszentrum (engl. ICC) dem Kehrwert der Krümmung $\kappa$ entspricht. 
...
Es ist allerdings auch eine Vereinfachung der Gleichung \ref{eq:lenkwinkel_complex} für sehr große Kurvenradien im Vergleich zu Fahrzeuglänge selbst möglich \cite{acker}:
